...
const GAME_THEMES: Record<GoalTimeframe, GameCardTheme> = {
  daily: {
    ...
    icon: (
      <Zap className="w-3 h-3 text-emerald-300 shrink-0 fill-emerald-300/30" />
    ),
    bgGradient: 'from-[#032e22] via-[#064e3b] to-[#021f17]',
    borderColor: 'border-emerald-500/40',
    glowColor: 'shadow-[0_4px_16px_rgba(5,150,105,0.2)]',
    activePillClass:
      'bg-emerald-400 text-slate-950 font-black shadow-[0_0_8px_rgba(52,211,153,0.6)]',
    inactivePillClass:
      'text-emerald-200/70 hover:text-white hover:bg-emerald-900/50',
    progressBarBg: 'bg-slate-950/70 border border-emerald-500/30',
    progressBarFill:
      'bg-gradient-to-r from-emerald-500 via-teal-300 to-emerald-200 shadow-[0_0_8px_rgba(52,211,153,0.8)]',
  ...
  weekly: {
    ...
    activePillClass:
      'bg-sky-400 text-slate-950 font-black shadow-[0_0_8px_rgba(56,189,248,0.6)]',
    ...
    progressBarFill:
      'bg-gradient-to-r from-sky-500 via-cyan-300 to-blue-200 shadow-[0_0_8px_rgba(56,189,248,0.8)]',
  ...
  monthly: {
    ...
    icon: (
      <Flame className="w-3 h-3 text-violet-300 shrink-0 fill-violet-300/30" />
    ),
    bgGradient: 'from-[#230b42] via-[#5b21b6] to-[#180630]',
    borderColor: 'border-violet-500/40',
    glowColor: 'shadow-[0_4px_16px_rgba(139,92,246,0.2)]',
    activePillClass:
      'bg-violet-400 text-slate-950 font-black shadow-[0_0_8px_rgba(167,139,250,0.6)]',
    inactivePillClass:
      'text-violet-200/70 hover:text-white hover:bg-violet-900/50',
    progressBarBg: 'bg-slate-950/70 border border-violet-500/30',
    progressBarFill:
      'bg-gradient-to-r from-violet-500 via-fuchsia-300 to-purple-200 shadow-[0_0_8px_rgba(167,139,250,0.8)]',
  ...
  yearly: {
    ...
    icon: (
      <Crown className="w-3 h-3 text-amber-300 shrink-0 fill-amber-300/30" />
    ),
    ...
    progressBarFill:
      'bg-gradient-to-r from-amber-500 via-yellow-300 to-amber-100 shadow-[0_0_8px_rgba(251,191,36,0.8)]',
...
const AnimatedCurrencyDisplay: React.FC<{ value: number }> = memo(
  ({ value }) => {
    ...
    return (
      <span
        ref={nodeRef}
        className="font-sans font-black tabular-nums tracking-tight"
      >
        ₱
...
export const SidebarProfileFlipper: React.FC<SidebarProfileFlipperProps> = ({
  ...
  // Role verification: SuperAdmin and Admin can view/flip to Goals
  const isSuperAdminUser = isSuperAdmin(user?.email);
  const isAdminUser = profile?.role === 'admin';
  const canViewGoals = isSuperAdminUser || isAdminUser;
  ...
  const timeframeKeys: GoalTimeframe[] = [
    'daily',
    'weekly',
    'monthly',
    'yearly',
  ...
  return (
    ...
    <motion.div
      ...
      animate={{ height: isFlipped && canViewGoals ? 148 : 92 }}
    ...
    >
      <div
        ...
        style={{
          ...
          transform:
            isFlipped && canViewGoals ? 'rotateY(180deg)' : 'rotateY(0deg)',
        }}
      >
        {/* ══════════════════════════════════════════════
            FRONT FACE: COMPACT PROFILE CARD
        ══════════════════════════════════════════════ */}
        <div
        ...
        >
          {/* Top Row: Avatar + Name + Goals Button (Visible only to Admin/Superadmin) */}
          <div className="flex items-center justify-between gap-2.5">
            <div className="relative shrink-0">
              <div
                className={`w-10 h-10 rounded-full ring-2 ${isSuperAdminUser ? 'ring-purple-500/50 dark:ring-purple-400/50' : 'ring-[#123c73]/20 dark:ring-red-500/30'} p-0.5 bg-slate-100 dark:bg-neutral-800 overflow-hidden`}
              >
                {avatarElement}
              </div>
            ...
            <div className="flex-1 min-w-0">
              <h4
                className={`font-heading text-xs font-black tracking-wider uppercase truncate leading-none ${isSuperAdminUser ? 'text-purple-600 dark:text-purple-400' : 'text-slate-900 dark:text-white'}`}
              >
                {isSuperAdminUser
                  ? 'SUPERADMIN'
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                >
                  <span
                    className={`w-1 h-1 rounded-full ${isSuperAdminUser ? 'bg-purple-500 animate-pulse' : 'bg-emerald-500'}`}
                  />
                  {isSuperAdminUser ? 'SUPERADMIN' : profile?.role || 'STAFF'}
                </span>
              </div>
            </div>
            ...
            {canViewGoals && (
              ...
              >
                ...
                <span className="text-[10px] font-sans font-black tracking-wider uppercase">
                  GOALS
                </span>
              </button>
            )}
        ...
        {/* ══════════════════════════════════════════════
            BACK FACE: INSTANT REVENUE GOALS HUD (ADMIN/SUPERADMIN ONLY)
        ══════════════════════════════════════════════ */}
        {canViewGoals && (
          <div
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 space-y-1.5">
                ...
                <div className="flex items-center justify-between gap-1">
                  ...
                  >
                    ...
                    <span className="text-[8px] font-sans font-black tracking-wider uppercase">
                      PROFILE
                    </span>
                  </button>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-white/90">
                    ...
                    <span className="text-[9px] font-mono font-bold text-white/90">
                      ...
                      {currentGoalTarget >= 1000
                        ? `${(currentGoalTarget / 1000).toLocaleString()}k`
                        : currentGoalTarget}
                    </span>
                  </div>
                  ...
                  <div className="flex items-baseline justify-between gap-1.5 mt-0.5">
                    ...
                    <div
                      className={`px-1.5 py-0.5 rounded-md border font-mono text-[10px] font-black tracking-tight shrink-0 ${
                        isGoalAchieved
                ...
                <div className="space-y-0.5">
                  <div
                    className={`w-full h-2.5 rounded-full overflow-hidden p-0.5 ${activeTheme.progressBarBg}`}
                  >
                    <motion.div
              ...
              <div className="grid grid-cols-2 gap-1.5 relative z-10 pt-1 border-t border-white/15">
                <div
                  className={`px-2 py-0.5 rounded-lg border backdrop-blur-md flex items-center justify-between ${activeTheme.chipBg} ${activeTheme.chipBorder}`}
                >
                  ...
                  <span className="text-[9px] font-mono font-bold text-white">
                    ₱{Math.round(logbookRevenue).toLocaleString()}
                  </span>
                </div>
                ...
                >
                  ...
                  <span className="text-[9px] font-mono font-bold text-white">
                    ₱{Math.round(salesRevenue).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
