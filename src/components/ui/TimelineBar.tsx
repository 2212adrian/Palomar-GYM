...
export const TimelineBar: React.FC<TimelineBarProps> = ({
  ...
  const isCurrentWeek = useMemo(() => {
    ...
    return (
      startOfDay(currentWeekStart).getTime() >=
          ...
          >
            ...
            <span className="font-heading text-xs sm:text-sm text-[#123c73] dark:text-slate-100 tracking-wider block mt-0.5 select-none font-extrabold">
              {role === 'admin'
                ...
                : format(
                    addDays(currentWeekStart, selectedDayIndex),
                    'EEEE, MMMM d, yyyy'
                  )}
            </span>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {(!isCurrentWeek || !isSelectedDateToday) && role === 'admin' && (
              ...
              >
                ...
                <span className="uppercase text-[9px] sm:text-[10px] tracking-wider font-heading font-extrabold">
                  Today
                </span>
              </button>
            )}
            ...
            >
              {DAYS_OF_WEEK.map((day: string, idx: number) => {
                ...
                return (
                  ...
                  >
                    <span className="text-[8.5px] sm:text-[9.5px] font-heading tracking-wider">
                      {day}
                    </span>
                    <span className="text-sm sm:text-base font-sans font-black mt-0.5">
                      {format(date, 'd')}
                    </span>
                    {isTodayDate && (
                      <span
                        className={`absolute bottom-1 w-1.5 h-1.5 rounded-full ${active ? 'bg-white' : 'bg-[#123c73] dark:bg-red-400'}`}
                      />
                    )}
                  </button>
                );
                  ...
                  >
                    {filterOptions.map((opt) => (
                      <option
                        key={opt.value}
                        value={opt.value}
                        className="bg-white dark:bg-[#1e232d] text-slate-800 dark:text-slate-200"
                      >
                    ...
                    >
                      {paymentOptions.map((opt) => (
                        <option
                          ...
                          className="bg-white dark:bg-[#1e232d] text-slate-800 dark:text-slate-200"
                        >
                          {opt.label === 'All Pay' || opt.label === 'All'
                            ? 'Pay: All'
                            : `Pay: ${opt.label}`}
                        </option>
                      ))}
