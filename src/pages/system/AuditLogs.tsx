...
export const AuditLogs: React.FC = () => {
  ...
  const formatActionName = (action: string): string => {
    ...
    if (act.includes('SALE_REMOVED') || act.includes('SALE_DELETE'))
      return 'Moved Sale to Recycle Bin';
    if (act.includes('SALE_RESTORED')) return 'Restored Sale from Recycle Bin';
    if (act.includes('SALES_REPORT') || act.includes('SALES_PRINT'))
      return 'Generated Sales Report';
    if (act.includes('RECEIPT_PRINTED')) return 'Printed Official Receipt';
    if (act.includes('PRODUCT_ADDED') || act.includes('PRODUCT_CREATE'))
      return 'Added New Product';
    if (act.includes('PRODUCT_UPDATED') || act.includes('PRODUCT_EDIT'))
      return 'Updated Product Information';
    if (act.includes('PRODUCT_REMOVED') || act.includes('PRODUCT_DELETE'))
      return 'Moved Product to Recycle Bin';
    if (act.includes('PRODUCT_RESTORED'))
      return 'Restored Product from Recycle Bin';
    ...
    if (
      ...
      act.includes('ATTENDANCE_CHECKIN')
    )
      return 'Logged Member / Walk-In Check-In';
    if (act.includes('CHECK_OUT')) return 'Member Checked Out';
    if (
      act.includes('LOGBOOK_REMOVED') ||
      act.includes('LOGBOOK_DELETE') ||
      act.includes('ATTENDANCE_REMOVED')
    )
      return 'Moved Check-In to Recycle Bin';
    if (act.includes('LOGBOOK_RESTORED') || act.includes('ATTENDANCE_RESTORED'))
      return 'Restored Check-In from Recycle Bin';
    if (act.includes('LOGBOOK_REPORT') || act.includes('ATTENDANCE_REPORT'))
      return 'Generated Attendance Report';
    ...
    if (
      ...
      return 'Approved Online Pre-Registration';
    if (
      act.includes('ONLINE_REGISTRATION_REJECTED') ||
      act.includes('ONLINE_REG_REJECTED')
    )
      return 'Rejected Online Pre-Registration';
    if (
      act.includes('ONLINE_REGISTRATION_PURGED') ||
      act.includes('ONLINE_REG_PURGED')
    )
      return 'Purged Expired Registrations';
    if (act.includes('MEMBER_CREATED') || act.includes('MEMBER_ENROLLED'))
      return 'Enrolled New Member';
    if (act.includes('MEMBER_UPDATED') || act.includes('MEMBER_EDIT'))
      return 'Updated Member Details';
    if (
      act.includes('MEMBER_REMOVED') ||
      act.includes('MEMBER_DELETE') ||
      act.includes('MEMBER_DEACTIVATED')
    )
      return 'Moved Member to Recycle Bin';
    if (act.includes('MEMBER_RESTORED'))
      return 'Restored Member from Recycle Bin';
    if (act.includes('MEMBER_CARDS_PRINTED')) return 'Printed Member ID Cards';
    if (
      act.includes('MEMBER_PLAN_EXTENDED') ||
      act.includes('SUBSCRIPTION_EXTENDED')
    )
      return 'Extended Member Subscription';

    // Incidents
    if (
      act.includes('INCIDENT_CREATED') ||
      act.includes('INCIDENT_REPORT_CREATED')
    )
      return 'Filed Incident Report';
    if (
      act.includes('INCIDENT_RESOLVED') ||
      act.includes('INCIDENT_REPORT_RESOLVED')
    )
      return 'Resolved Incident Report';
    if (act.includes('INCIDENT_UPDATED') || act.includes('INCIDENT_STATUS'))
    ...
    if (act.includes('RESTORE_DATABASE') || act.includes('RESTORE_BACKUP'))
      return 'Restored System Backup';
    if (act.includes('LOGIN') || act.includes('SIGN_IN'))
      return 'User Logged In';
    if (act.includes('LOGOUT') || act.includes('SIGN_OUT'))
      return 'User Logged Out';
    if (act.includes('USER_CREATED') || act.includes('ACCOUNT_CREATED'))
  ...
  const getActionIcon = (action: string, severity: string) => {
    ...
    if (act.includes('ONLINE_REGISTRATION_REJECTED'))
      return <UserX className={`${style} text-red-500`} />;
    if (act.includes('INCIDENT'))
      return <AlertTriangle className={`${style} text-amber-500`} />;
    if (
      act.includes('SALE') ||
      act.includes('PRODUCT') ||
      act.includes('RECEIPT')
    )
      return <CreditCard className={`${style} text-emerald-500`} />;
    if (severity === 'critical')
      return <Trash2 className={`${style} text-red-500`} />;
    if (
      act.includes('CHECK_IN') ||
      act.includes('CHECK_OUT') ||
      act.includes('LOGBOOK') ||
    ...
    if (act.includes('PAYMENT') || act.includes('RATES'))
      return <CreditCard className={`${style} text-emerald-500`} />;
    if (
      act.includes('MEMBER') ||
      act.includes('USER') ||
      ...
      return <User className={`${style} text-blue-400`} />;
    if (act.includes('BACKUP') || act.includes('SNAPSHOT'))
      return <Database className={`${style} text-amber-500`} />;
    if (act.includes('RESTORE'))
      return <RefreshCw className={`${style} text-red-500`} />;
    if (act.includes('LOGIN'))
      return <LogIn className={`${style} text-blue-400`} />;
    if (act.includes('LOGOUT'))
      return <LogOut className={`${style} text-slate-400`} />;
    if (
      act.includes('CONFIG') ||
      act.includes('RATES') ||
      act.includes('GYM') ||
      ...
      return <SettingsIcon className={`${style} text-amber-500`} />;
    return <Activity className={`${style} text-slate-400`} />;
  };

  const getDayLabel = (dateStr: string): string => {
    const ManilaTimeStr = new Date(dateStr).toLocaleString('en-US', {
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const criticalCount = logs.filter(
      (l) => getActionSeverity(l.action) === 'critical'
    ).length;

    const todayPHTStr = new Date().toLocaleDateString('en-US', {
      timeZone: 'Asia/Manila',
    });
    const todayCount = logs.filter((l) => {
      const logPHTStr = new Date(l.created_at).toLocaleDateString('en-US', {
        timeZone: 'Asia/Manila',
      });
      return logPHTStr === todayPHTStr;
    }).length;

    const backupCount = logs.filter(
      (l) => getActionCategory(l.action) === 'backups'
    ).length;
    const authCount = logs.filter(
      (l) => getActionCategory(l.action) === 'auth'
    ).length;
  ...
  const processedLogs = useMemo(() => {
    return logs.filter((log) => {
      const matchesCategory =
        activeCategory === 'all' ||
        getActionCategory(log.action) === activeCategory;
      const matchesSeverity =
        activeSeverity === 'all' ||
        getActionSeverity(log.action) === activeSeverity;
      const matchesOperator =
        activeOperator === 'all' || log.actor_username === activeOperator;
      ...
      return (
        matchesCategory && matchesSeverity && matchesOperator && matchesSearch
      );
    });
  ...
  const timelineGroups = useMemo(() => {
    ...
    processedLogs.forEach((log) => {
      ...
      if (!currentGroup) {
        ...
        const lastInGroupTime = new Date(
    ...
    <div className="w-full space-y-6 font-body text-(--color-text) pr-1 pl-1 sm:px-0 min-w-0">
      ...
      <div className="hidden md:block group/kpis relative transition-all duration-500 ease-in-out border border-transparent hover:border-(--border-color)/40 rounded-3xl p-1">
        <div className="flex items-center justify-between px-4 py-2.5 bg-slate-500/5 border border-dashed border-(--border-color) rounded-2xl text-xs font-bold text-slate-400 uppercase tracking-wider cursor-pointer transition-all hover:bg-slate-500/10">
          ...
          <span className="text-[9px] text-slate-500 font-bold group-hover/kpis:hidden">
            Hover to Expand metrics
          </span>
          <span className="text-[9px] text-slate-500 font-bold hidden group-hover/kpis:inline">
            Collapse metrics
          </span>
        </div>
                ...
                <div className="space-y-3 pl-6 sm:pl-10">
                  {dayGroup.items.map((group: any) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`absolute hidden sm:block sm:left-[-22px] top-5.5 w-3 h-3 rounded-full border-2 bg-(--bg-page) transition-all ${
                          ...
                          <div className="flex items-start gap-2.5 sm:gap-4 flex-1 min-w-0">
                            ...
                            <div className="space-y-0.5 text-left flex-1 min-w-0">
                              ...
                              <div className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed break-words">
                                <span className="font-bold text-slate-800 dark:text-slate-200 mr-1.5 break-all">
                            ...
                            {itemCount > 1 ? (
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold">
                                <div className="space-y-1">
                                  <span className="text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[9px] block">
                                    Actor Username
                                  </span>
                                  <span className="text-slate-800 dark:text-slate-100 font-bold block break-all">
                                    {representative.actor_username}
                                  </span>
                                </div>
                                <div className="space-y-1">
                                  <span className="text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[9px] block">
                                  ...
                                  <span className="text-slate-800 dark:text-slate-100 font-mono text-[10px] block break-all">
                                ...
                                <div className="space-y-1 sm:col-span-2">
                                  ...
                                  <div className="text-slate-700 dark:text-slate-200 font-normal leading-relaxed block bg-(--bg-page) p-3 rounded-lg border border-(--border-color) text-xs whitespace-pre-wrap text-left select-all break-words">
                                    {renderFormattedAuditDetails(
                                      representative.details ||
                                        'No extended metadata payload registered.'
                                    )}
                                  </div>
                                </div>
                              </div>
            ...
            {totalPages > 1 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-(--bg-card) border border-(--border-color) rounded-2xl p-4 shadow-xs font-semibold">
                <span className="text-xs text-slate-400 text-center sm:text-left">
                  Showing{' '}
                  <strong className="text-slate-550">
                    {(currentPage - 1) * itemsPerPage + 1}
                  </strong>{' '}
                  to{' '}
                  <strong className="text-slate-550">
                    {Math.min(currentPage * itemsPerPage, totalGroupsCount)}
                  </strong>{' '}
                  of{' '}
                  <strong className="text-slate-550">{totalGroupsCount}</strong>{' '}
                  grouped items
                </span>
                <div className="flex items-center gap-2">
                  <button
      ...
      <div className="flex items-start gap-4 p-4 sm:p-5 bg-blue-500/5 border border-blue-500/20 rounded-2xl text-xs text-blue-700 dark:text-blue-300 leading-normal max-w-full">
        ...
        <div className="space-y-1.5 text-left">
          <p className="font-heading tracking-wider uppercase text-xs">
            System Retention Policy (pgAudit Aligned)
          </p>
          <p className="text-[11px] font-semibold text-slate-400 opacity-90 leading-relaxed">
            Audit logging operations are protected under secure write-only
            database constraints. Manual record updates or deletions are
            restricted at the database catalog layer. Logs are retained for
            exactly one year and pruned daily via an automated database cron job
            at midnight Manila time.
          </p>
        </div>
      </div>
    </div>
