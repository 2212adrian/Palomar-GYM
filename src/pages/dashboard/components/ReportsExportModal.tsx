...
export const ReportsExportModal: React.FC<ReportsExportModalProps> = ({
  ...
  const normalizedInitial = (
    initialType === 'inventory' ? 'sales' : initialType
  ) as ReportCategoryType;
  const [reportType, setReportType] =
    useState<ReportCategoryType>(normalizedInitial);
  ...
  const [previewTab, setPreviewTab] = useState<'showcase' | 'details'>(
    'showcase'
  );
    ...
    () => [
      {
        ...
        legalPurpose:
          'Mandatory bookkeeping ledger for BIR quarterly percentage tax filing, audit inspection, and official receipt verification.',
        columns: [
          'OR / Ref #',
          'Date & Time',
          'Customer Name',
      ...
      },
      {
        ...
        description:
          'Itemized retail sales ledger capturing unit prices, barcode identities, quantity velocities, gross sales per item, and current shelf stock levels.',
        legalPurpose:
          'Point-of-sale inventory audit, shrinkage monitoring, merchandise margin analysis, and reorder planning.',
        columns: [
          'Barcode ID',
          'Product Name',
          'Unit Price (₱)',
      ...
      },
      {
        ...
        description:
          'Comprehensive check-in logbook aggregating member and walk-in visits into a unified customer pass summary with visit counts and total fees.',
        legalPurpose:
          'Facility utilization verification, front-desk collection reconciliation, and physical safety occupancy tracking.',
        columns: [
          'Customer Name',
          'Access Category',
          'Total Visits (Qty)',
      ...
      },
      {
        ...
        description:
          'Gym membership contracts, subscription intakes, recurring renewal revenue, plan packages, and active client terms.',
        legalPurpose:
          'Membership recurring revenue reporting, contract expiration audit, and customer account validation.',
        columns: [
          'Contract ID',
          'Member Name',
          'Plan Name',
      ...
      },
      {
        ...
        description:
          'Unified financial timeline combining retail point-of-sale proceeds, logbook day passes, and membership subscriptions into an executive financial statement.',
        legalPurpose:
          'Executive management review, daily cash-up reconciliation, and multi-stream revenue growth analytics.',
        columns: [
          'Date',
          'Sales (₱)',
          'Logbook (₱)',
  ...
  const selectedCategoryMeta = useMemo(() => {
    return (
      reportCategories.find((c) => c.id === reportType) || reportCategories[0]
    );
  ...
  const loadCategoryData = useCallback(async () => {
    ...
    try {
      ...
      if (reportType === 'bir') {
        ...
        (attRes.data || [])
          .filter((a: any) => Number(a.entry_fee || 0) > 0)
          .forEach((a: any) => {
        ...
        (subsRes.data || []).forEach((sub: any) => {
          ...
          items.push({
            receipt_no:
              sub.receipt_number || `SUB-${String(sub.id).slice(0, 6)}`,
            date: sub.created_at,
        ...
        allSales.forEach((s) => {
          ...
          if (s.items) {
            ...
            if (Array.isArray(rawItems) && rawItems.length > 0) {
              rawItems.forEach((i: any) => {
                ...
                const price = Number(
                  i.price || i.selling_price || i.unit_price || 0
                );
                const sub = Number(i.subtotal || price * qty || 0);
                itemsParsed.push({
                  id: pid,
                  name: pname,
                  barcode: i.barcode_id || i.barcode,
                  qty,
                  unitPrice: price,
                  subtotal: sub,
                });
          ...
          if (itemsParsed.length === 0 && s.product_name) {
            ...
            if (itemsParsed.length > 0 && totalAmount > 0) {
              const totalUnits =
                itemsParsed.reduce((sum, it) => sum + it.qty, 0) || 1;
          ...
          itemsParsed.forEach((item) => {
            ...
            if (target) {
              ...
              target.revenue +=
                item.subtotal > 0 ? item.subtotal : target.price * item.qty;
            } else if (item.name) {
              ...
              prodMap[fallbackKey].revenue +=
                item.subtotal > 0
                  ? item.subtotal
                  : prodMap[fallbackKey].price * item.qty;
            }
          });
        });

        const list = Object.values(prodMap).sort(
        ...
        const aggregatedList = Array.from(aggregatedMap.values()).sort(
          (a, b) =>
            b.visits_count - a.visits_count || b.total_fees - a.total_fees
        );
        ...
        const [salesRes, attRes, subsRes] = await Promise.all([
          ...
          supabase
            .from('attendance')
            .select('check_in_time, entry_fee')
            .is('deleted_at', null)
            .gte('check_in_time', startIso)
            .lte('check_in_time', endIso),
          supabase
        ...
        const combinedList = Array.from(dayMap.values()).sort((a, b) =>
  ...
  const handleQuickPreset = (
    preset: 'today' | 'week' | 'month' | 'last_month' | 'year'
  ) => {
    const now = new Date();
    let s = now;
    let e = now;
  ...
  const categoryStats = useMemo(() => {
    if (reportType === 'bir') {
      ...
      const gross = liveAttendanceData.reduce(
        (acc, a) => acc + Number(a.total_fees || 0),
        0
      );
      const totalVisits = liveAttendanceData.reduce(
        (acc, a) => acc + Number(a.visits_count || 1),
        0
      );
      return {
      ...
      const gross = liveSubsData.reduce(
      ...
      const gross = liveCombinedData.reduce(
        (acc, r) => acc + r.totalRevenue,
        0
      );
      const totalTx = liveCombinedData.reduce(
        (acc, r) => acc + r.transactionCount,
        0
      );
      return {
    ...
    try {
      ...
      if (reportType === 'bir') {
        ...
        liveSubsData.forEach((s: any) => {
          const cid =
            s.receipt_number ||
            (s.id ? `SUB-${String(s.id).slice(0, 6)}` : 'N/A');
  ...
  const handleExportPDF = async () => {
    try {
      ...
      page.drawText(
        `Compliance Standard: ${selectedCategoryMeta.badge} • Date Generated: ${format(new Date(), 'yyyy-MM-dd HH:mm')}`,
        {
          x: 40,
          y: y - 42,
          size: 7,
      ...
      page.drawText(`Reporting Period: ${startDate} to ${endDate}`, {
        x: 35,
        y: y - 13,
        size: 8,
        font: fontBold,
      ...
      page.drawText(`Total Records: ${categoryStats.count}`, {
        x: 35,
        y: y - 23,
        size: 7.5,
        font,
      ...
      page.drawText(
        `Total Value: PHP ${categoryStats.totalValue.toLocaleString('en-PH', { minimumFractionDigits: 2 })}`,
        {
          x: width - 260,
          y: y - 13,
          size: 8.5,
      ...
      page.drawText(`Scope: ${categoryStats.secondary}`, {
        x: width - 260,
        y: y - 23,
        size: 7.5,
        font,
      ...
      if (reportType === 'bir') {
        ...
        liveBirData.forEach((row, idx) => {
          ...
          page.drawText(String(row.receipt_no).slice(0, 15), {
            x: 30,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(String(row.date).slice(0, 16), {
            x: 130,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(String(row.customer_name).slice(0, 22), {
            x: 235,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(String(row.transaction_type).slice(0, 14), {
            x: 375,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(row.gross_sales.toFixed(2), {
            x: 460,
            y,
            size: 6.5,
            font: fontBold,
          ...
          page.drawText(row.vat_exempt_sales.toFixed(2), {
            x: 540,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(row.vat_amount.toFixed(2), {
            x: 615,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(row.net_sales.toFixed(2), {
            x: 685,
            y,
            size: 6.5,
            font: fontBold,
          ...
          page.drawText(String(row.payment_method), {
            x: 745,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(String(row.status), {
            x: 790,
            y,
            size: 6.5,
            font: fontBold,
            color: rgb(0.06, 0.6, 0.35),
          });
          y -= 13;
        });
        ...
        liveSalesData.forEach((row, idx) => {
          ...
          page.drawText(String(row.barcode).slice(0, 18), {
            x: 30,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(String(row.name).slice(0, 42), {
            x: 150,
            y,
            size: 6.5,
            font: fontBold,
          ...
          page.drawText(row.revenue.toFixed(2), {
            x: 630,
            y,
            size: 6.5,
            font: fontBold,
          ...
          page.drawText(`${row.stock} in stock`, {
            x: 740,
            y,
            size: 6.5,
            font,
        ...
        liveAttendanceData.forEach((row: any, idx: number) => {
          ...
          page.drawText(String(row.customer_name).slice(0, 32), {
            x: 30,
            y,
            size: 6.5,
            font: fontBold,
          ...
          page.drawText(String(row.customer_type || 'Walk-In'), {
            x: 250,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(`${row.visits_count} visits`, {
            x: 410,
            y,
            size: 6.5,
            font: fontBold,
          ...
          page.drawText(Number(row.total_fees || 0).toFixed(2), {
            x: 540,
            y,
            size: 6.5,
            font: fontBold,
          ...
          page.drawText(String(row.latest_checkin).slice(0, 16), {
            x: 680,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(String(row.payment_method || 'Cash'), {
            x: 770,
            y,
            size: 6.5,
            font,
        ...
        liveSubsData.forEach((row: any, idx: number) => {
          ...
          page.drawText(
            String(row.members?.full_name || 'Member').slice(0, 24),
            { x: 150, y, size: 6.5, font: fontBold, color: rgb(0.1, 0.1, 0.1) }
          );
          page.drawText(
            String(
              row.plan_type ? row.plan_type.toUpperCase() : 'Monthly Pass'
            ).slice(0, 22),
            { x: 320, y, size: 6.5, font, color: rgb(0.2, 0.2, 0.2) }
          );
          ...
          page.drawText(String(row.start_date || row.created_at).slice(0, 10), {
            x: 590,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(String(row.end_date || 'Ongoing').slice(0, 10), {
            x: 680,
            y,
            size: 6.5,
            font,
        ...
        liveCombinedData.forEach((row, idx) => {
          ...
          page.drawText(row.sales.toFixed(2), {
            x: 160,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(row.logbook.toFixed(2), {
            x: 300,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(row.subscription.toFixed(2), {
            x: 440,
            y,
            size: 6.5,
            font,
          ...
          page.drawText(`${row.transactionCount} transactions`, {
            x: 725,
            y,
            size: 6.5,
            font,
      ...
      // Footer
      page.drawText(
        'Palomar Gym System 2.0 • Official Compliance & Audit Export',
        {
          x: 300,
          y: 15,
          size: 7,
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <Icon
                      className={`w-4 h-4 ${isSelected ? 'text-[#123c73] dark:text-blue-400' : 'text-slate-400'}`}
                    />
                    {isSelected && (
                  ...
                  <div>
                    <span className="text-xs font-bold font-heading block leading-snug">
                      {cat.shortLabel}
                    </span>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-[11px] font-mono text-slate-500">
                  ...
                  <strong className="text-slate-700 dark:text-slate-300 font-mono">
                    {selectedCategoryMeta.filePrefix}.csv
                  </strong>
                </span>
              </div>
              ...
              <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-xs max-h-48 overflow-y-auto">
                <table className="w-full text-left text-[11px] border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-zinc-900 text-slate-700 dark:text-slate-300 font-body">
                      ...
                      liveBirData.slice(0, 5).map((row, idx) => (
                        ...
                        >
                          ...
                          <td className="p-2.5 text-slate-500">
                            ₱{row.vat_exempt_sales.toFixed(2)}
                          </td>
                          <td className="p-2.5 text-slate-500">
                    ...
                    {reportType === 'sales' &&
                      liveSalesData.slice(0, 5).map((row, idx) => (
                        <tr
                          ...
                          className="hover:bg-slate-50 dark:hover:bg-zinc-900/50"
                        >
                          <td className="p-2.5 font-mono text-xs font-bold text-blue-600 dark:text-blue-400">
                            {row.barcode}
                          </td>
                    ...
                    {reportType === 'attendance' &&
                      liveAttendanceData.slice(0, 5).map((row, idx) => (
                        ...
                        >
                          ...
                          <td className="p-2.5">
                            <span className="px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-300 text-[10px] font-bold">
                              {row.customer_type}
                            </span>
                          </td>
                          <td className="p-2.5 font-extrabold text-[#123c73] dark:text-blue-400">
                            {row.visits_count} visits
                          </td>
                          <td className="p-2.5 font-bold text-emerald-600 dark:text-emerald-400">
                            ₱{Number(row.total_fees || 0).toFixed(2)}
                          </td>
                          <td className="p-2.5 text-slate-500 whitespace-nowrap">
                            {String(row.latest_checkin).slice(0, 16)}
                          </td>
                          <td className="p-2.5">
                    ...
                    {reportType === 'subscriptions' &&
                      liveSubsData.slice(0, 5).map((row, idx) => (
                        <tr
                          ...
                          className="hover:bg-slate-50 dark:hover:bg-zinc-900/50"
                        >
                          <td className="p-2.5 font-mono text-xs font-bold text-purple-600 dark:text-purple-400">
                            {row.receipt_number ||
                              `SUB-${String(row.id).slice(0, 6)}`}
                          </td>
                          <td className="p-2.5 font-bold">
                          ...
                          <td className="p-2.5 text-slate-500">
                            {String(row.start_date || row.created_at).slice(
                              0,
                              10
                            )}
                          </td>
                          <td className="p-2.5 text-slate-500">
                            {String(row.end_date || 'Ongoing').slice(0, 10)}
                          </td>
                          <td className="p-2.5">
                            {row.payment_method || 'Cash'}
                          </td>
                    ...
                    {reportType === 'combined' &&
                      liveCombinedData.slice(0, 5).map((row, idx) => (
                        <tr
                          key={idx}
                          className="hover:bg-slate-50 dark:hover:bg-zinc-900/50"
                        >
                          ...
                          <td className="p-2.5 font-mono">
                            ₱{row.logbook.toFixed(2)}
                          </td>
                          <td className="p-2.5 font-mono">
                            ₱{row.subscription.toFixed(2)}
                          </td>
                          <td className="p-2.5 font-bold text-emerald-600 dark:text-emerald-400">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div className="space-y-2 p-3 bg-white dark:bg-zinc-950 rounded-xl border border-slate-200 dark:border-zinc-800">
                ...
                <div className="pt-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 font-mono text-[10px] font-bold">
                      .CSV (UTF-8 Excel)
                    </span>
                    <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 font-mono text-[10px] font-bold">
                      .PDF (Landscape A4)
              ...
              <div className="space-y-2 p-3 bg-white dark:bg-zinc-950 rounded-xl border border-slate-200 dark:border-zinc-800">
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {selectedCategoryMeta.columns.map((col, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-1 rounded-md bg-slate-100 dark:bg-zinc-800 text-[10px] font-mono text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-zinc-700"
                    >
                      {col}
                    </span>
                  ))}
